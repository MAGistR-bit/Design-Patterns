...
> **_Паттерн Наблюдатель связан с паттерном Публикация-Подписка_**,
> предназначенным для более сложных ситуаций с несколькими
> субъектами и (или) типами сообщений.
...
Существует и другой способ обработки данных в субъекте:
положиться на то, что наблюдатели будут получать их
от субъекта по мере надобности.  
То есть, нужно изменить код так, чтобы при получении уведомления
об изменениях наблюдатель вызывал бы `get-методы` субъекта
для получения требуемых изменений.  
Программный код находится в этом [пакете](weathespullversion).